...
# BUILD_FLAGS are passed to all compilation units (including framework libs like Wire).
try:
        ...
        for name in os.listdir(pkg_dir):
            if name.startswith("framework-arduinoadafruitnrf52"):
                ...
                if os.path.isdir(libs):
                    ...
                    # So that #include <SPI.h> resolves to libraries/SPI/SPI.h
...
# so Host MSC (and SdFat's File) are not included, avoiding File ambiguity with LittleFS.
try:
